...
def init_parallel():
    print("There are " + str(cpu_count()) + "cores, of which" + str(len(sched_getaffinity(0))) + "are available.")
...
def determine_terms(A, Z, bottom, top, init_alpha=1.0, min_alpha=2**(-9), try_lstsq=False, max_iter=2**12):
    if (bottom > top):
        raise ValueError("Error! low must be less than or equal to high, but you gave" + str(low) + "and" + str(high) +".")
    low, high = bottom, top
...
def main(input1, input2, args):
    ...
    log("Dependent_Data is an array of length" + str(len(Dependent_Data)) + "with first elements:\n" + str(Dependent_Data[:5]) + "\n", file=args.logFile)
    log("Independent_Data is a length-" + str(len(Independent_Data)) + "array of arrays with first element:\n" + str(Independent_Data[0]) + "\n", file=args.logFile)    
    ...
    log("Independent_Data post-scaling is an array of arrays with first element:\n" + str(Independent_Data[0]) + "\n", file=args.logFile)
    ...
    if args.model=="SBM":
        ...
        args.cvIters = 1
    log("Each local model will be generated with" + str(args.k) + "nearest neighbors.\n", file=args.logFile)
    ...
    log("It took" +  str(time() - t0) + "seconds to perform model_at_point on all the evaluation points.\n", file=args.logFile)
...
if __name__ == "__main__":    
    ...
    log("\n" + str(len(original_values)) + "lines of original data have been loaded from" + str(args.train) + ".\n", file=args.logFile)
    values_to_model = np.loadtxt(args.eval, delimiter=args.delimiter, skiprows=args.headerRows)
    log(str(len(values_to_model)) + "lines of evaluation data have been loaded from" + str(args.eval) + ".\n", file=args.logFile)
    ...
    if not args.out:
        args.out=str(args.train.split('/')[-1]) + "_e-" + str(args.eval.split('/')[-1]) + "_i-" + str(args.depIndex) + "_s-" + str(args.skipVars) + "_v-" + str(args.variables) + "_m-" + str(args.model) + "_k-" + str(args.k) + "_D-" + str(args.degree) + "_L-" + str(args.Lasso) + "_R-" + str(args.randIters) + ".csv"

    np.savetxt(args.out, output, delimiter=",", fmt='%.17f')

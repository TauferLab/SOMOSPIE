...
def topo_fetch(topos=["Slope"], agg_fact=1):
    ...
    for t in topos:
        ...
        if not os.path.exists(topo_file):
            bash([f"{SUB_DATA}/fetch_topo_predictors.sh", topo_folder, t])
            pre_file = f"{topo_folder}{t}.sdat"
            if agg_fact>1:
                print(f"Scaling {t} to be more coarse, mean-aggragating by a factor of {agg_fact}.")
                bash([f"{SUB_CODE}/coarsify.R",  pre_file, topo_file, agg_fact])
                pre_file = topo_file
            bash([f"{SUB_CODE}/reproject_raster.R", pre_file, topo_file])
...
def topo_stack(topos=["Slope"], agg_fact=1, out_file=""):
    ...
    for t in topos:
        ...
        stack_args = [f"{SUB_CODE}/make_raster_stack.R", topo_folder, stack_file, "stack.tif", topo_file]
        if t==topos[0]:
            stack_args.pop(-2)
        bash(stack_args)
...
def topo_widget(topos=topo_options):
    ...
    for i in range(0,len(topo_acc.children[0].children),2):
...
def region_widget(reg_type_acc):
    ...
    if (("CEC" in region_types) and (not os.path.exists(f"{SUB_DATA}/TerrestrialEcoregions_L2_Shapefile"))) \
      or (("NEON" in region_types) and (not os.path.exists(f"{SUB_DATA}/NEONDomains_0"))):
        bash([f"{SUB_DATA}/fetch_ecoregions.sh"])

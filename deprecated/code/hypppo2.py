...
# Like ../hyppo_testing/hypppo2.py, but with two things added:
#   logging
#   firm upper and lower bound
...
import os
from __utils import log

# Parallelization.
def init_parallel(log_file=""):
    log(f"There are {os.cpu_count()} cores, of which {len(os.sched_getaffinity(0))} are available.\n", file=log_file)
...
def main(input1, input2, args):
    ...
    if scale:
        if len(scale)!=indepCount:
            log("Error: scale was specified, but isn't the same length as the sepcified number of independent variables!\n", file=args.log)
        scale = np.array(scale)
    else:
        scale = 1/np.std(Independent_Data, axis=0)
    log(f"Scaling factors:\n{scale}\n", file=args.log)
...
if __name__ == "__main__":    
    ...
    parser.add_argument( "-l", "--log", help="Name of file where logging is to be stored.", default="")

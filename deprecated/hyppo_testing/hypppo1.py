...
def model_at_point(x, Independent_Data, Dependent_Data, K, maxDegree, model="HYPPO", norm=2):
...
def main(input1, input2, depIndex=2, model="HYPPO", maxDegree=4, k=6, indepStart=0, indepCount=2, scale=[], norm=2, parallel=False):
    ...
    def MaP(x):
        ...
        local_model = model_at_point(x, Independent_Data, Dependent_Data, K, maxDegree, model, norm)
...
if __name__ == "__main__":    
    ...
    ### Commandline test
    ### ./hypppo1.py ../data/2012/t-postproc/8.5.csv -e ../data/2012/e-postproc/8.5.csv -o out1.csv -v3 -s2 -p1 -D5
...
output = main(original_values, values_to_model, depIndex=args.depIndex, model=args.model, maxDegree=args.degree, 
              k=args.k, indepStart=args.skipVars, indepCount=args.variables, parallel=args.parallel, scale=scale, norm=args.norm)

...
if __name__ == "__main__":    
    ...
    parser.add_argument( "-F", "--Flatten", help="Allow for sublinear models and lower degree by raising all predictors to a power p<1", type=float, default=0)
...
if not args.out:
    args.out=f"{args.fileName.split('/')[-1]}_e-{args.eval.split('/')[-1]}_i-{args.depIndex}_s-{args.skipVars}_v-{args.variables}_F-{args.Flatten}_m-{args.model}_k-{args.k}_D-{args.degree}_L-{args.Lasso}_p-{args.parallel}.csv"

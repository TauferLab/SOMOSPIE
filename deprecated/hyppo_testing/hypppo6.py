...
def determine_terms(A, Z, bottom, top, init_alpha=1.0, min_alpha=2**(-9), try_lstsq=False, max_iter=2000):
    if (bottom > top):
...
def determine_coefficients(independent_variable_points, dependent_variable_values, degree, lasso=0, rand=0):
    ...
    #  we can solve the system with the least-squares method
        ...
        # ... or random term selection...
            ...
            ## Warning! This doesn't quite do what we want.
            ## This only takes the one polynomial with the lowest SSE during cross-validation
            ##  but ignores the others generated from leaving out other points, 
            ##  even though they are relevant to the winning cummulative SSE.
            #if type(rand) != int:
...
#def leave_one_out_crossvalidation(independent_data_points, dependent_data_points, D, lasso, iter_rand):
#    return crossvalidation(independent_data_points, dependent_data_points, len(independent_data_points), 1, D, lasso, iter_rand)
...
def determine_model_degree(indep, dep, args):
    if args.model=="KNN":
        ...
        degree_with_errors = crossvalidation(indep, dep, args)#.k, 10, args.degree + 1, args.Lasso, args.randIters)

    #elif args.model=="HYPPO":
    #    degree_with_errors = leave_one_out_crossvalidation(indep, dep, args)#.degree + 1, args.Lasso, args.randIters)
...
def model_at_point(x, Independent_Data, Dependent_Data, args):
    ...
    indices_of_nearest_neighbors = indices_of_NNs(Independent_Data, x, args.k, norm=args.norm)
    ...
    if args.memoize and (neighborhood in stored_coefs):
        ...
        selected_indep_data = [ Independent_Data[i] for i in indices_of_nearest_neighbors ]
        selected_dep_data = [ Dependent_Data[i] for i in indices_of_nearest_neighbors ]
...
def main(input1, input2, args):
    ...
    # Set up global dictionaries for optional data dumps.
    if args.memoize:
        global stored_coefs
        stored_coefs = {}
    if args.errorFile:
    ...
    #  to produce the prediction at that point.
    def MaP(x):
        coords = list(x[:2])
        x = ((np.array(x[indepStart:indepStart+indepCount]) - shift)*scale)**args.Flatten
        prediction = model_at_point(x, Independent_Data, Dependent_Data, args)
...
if __name__ == "__main__":    
    ...
    parser.add_argument("-v", "--variables", type=int, default=2, 
                        help="Number of independent variables--i.e. number of predictor columns (default: %(default)s).")
    ...
    parser.add_argument("-M", "--memoize", type=int, default="1", 
                        help="Store results for a given set of nearest neighbors in a global dictionary (default: %(default)s).")
    parser.add_argument("-H", "--nbrhdFile", default="", 
                        help="The path for neighborhoods; will not compute if empty string (default). Doesn't work with -p1.")

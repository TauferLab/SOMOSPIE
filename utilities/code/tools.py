...
Models = {'KKNN' : f'../SOMOSPIE/code/ml_models/2b-kknn.R',
          'SBM'   :f'../SOMOSPIE/code/ml_models/hypppo7.py',
          'HYPPO' :f'../SOMOSPIE/code/ml_models/hypppo7.py',
          'RF'    :f'../SOMOSPIE/code/ml_models/2c-rf.R'
}
...
def mergin_raster(List_of_Rasters):
    ...
    if n == 1:
        ...
        commands = [f'{Path}/code/Mosaicking.R', List_of_Rasters[0], List_of_Rasters[1], 'Temp_Mosaicking.img']
        bash(commands)

        List_of_Rasters = List_of_Rasters[2:]
        for list in List_of_Rasters:
            print(list)
            commands = [f'{Path}/code/Mosaicking.R', 'Temp_Mosaicking.img', list, 'Temp_Mosaicking.img']
            bash(commands)
...
def croping_region(Raster_input, min_long, max_long, min_lat, max_lat, Raster_output):
    ...
    command = [f'{Path}/code/CroppingRegion.R', Raster_input, min_long, max_long, min_lat, max_lat, Raster_output]
    bash(command)
...
def convert_to_sdat(Raster):
    ...
    command = [f'{Path}/code/Change_Raster_Format.R', Raster, Raster[0:-4]+'.sdat']
    bash(command)
 
 ##################################################################################################################  

def terrestrial_parameters(name_file):
    command = [f'{Path}/code/terrestrial_parameters.sh', name_file]
    bash(command)
...
def changing_to_utm(Input_file, Output_file):
    command = [f'{Path}/code/CSVLATLONG_2_CSVUTM.R', Input_file, Output_file]
    bash(command)
